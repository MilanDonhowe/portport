...
#       of the portport server and portport client occurs, and therefore mandates some degree of non-ambiguous message passing,
#       as we actually have to care about what the packet contents between the portport server and portport client look like.
import ssl
import socket
import selectors
import threading
...
def start_relay_mgmt_server(service_close: threading.Event, port: int, key_file: str, cert_file: str, passphrase: str, auth_token: str, auth_bypass: bool = False, port_range: None|tuple[int,int] = None):
    ...
    def add_to_conn_table(listening_socket: socket.socket, mask: int):
        if mask & selectors.EVENT_READ:
            ...
            connection_table[addr]= threading.Thread(target=relayMgmt, args=(inbound_ssl_socket, service_close,port_range,))
    ...
    accept_selector.register(main_server, selectors.EVENT_READ)
    while not service_close.is_set():
